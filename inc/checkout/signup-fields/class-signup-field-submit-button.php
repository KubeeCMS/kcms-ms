...
class Signup_Field_Submit_Button extends Base_Signup_Field {
	...
	public function to_fields_array($attributes) {
		...
		if ($attributes['enable_go_back_button']) {
			...
			$is_second_step = isset($steps[1]) && $steps[1]['id'] === $attributes['step'];

			$is_plan_pre_selected = wu_request('wu_preselected') === 'products';

			$types_to_hide = array(
				'pricing_table',
			);

			$last_step_first = $steps[0]['fields'][0];

			$hide_when_preselected = isset($last_step_first["hide_{$last_step_first['type']}_when_pre_selected"]) ? $last_step_first["hide_{$last_step_first['type']}_when_pre_selected"] : false;

			$hide_first = !isset($steps[0]['fields'][2]) && in_array($last_step_first['type'], $types_to_hide, true) && (bool) $hide_when_preselected && $is_plan_pre_selected;

			$hide_go_back = $is_second_step && $hide_first;
