...
final class ArgumentDeclaration implements SassNode
{
    ...
    public function verify(int $positional, array $names) : void
    {
        ...
        if ($positional > \count($this->arguments)) {
            $message = \sprintf('Only %d %sargument%s allowed, but %d %s passed.', \count($this->arguments), empty($names) ? '' : 'positional ', \count($this->arguments) === 1 ? '' : 's', $positional, $positional === 1 ? 'was' : 'were');
