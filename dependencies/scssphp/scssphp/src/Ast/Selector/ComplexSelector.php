...
final class ComplexSelector extends Selector
{
     ...
     */
    public static function parse(string $contents, ?LoggerInterface $logger = null, ?string $url = null, bool $allowParent = \true) : ComplexSelector
    {
    ...
    public function withAdditionalCombinators(array $combinators, bool $forceLineBreak = \false) : ComplexSelector
    {
        ...
        return new ComplexSelector($this->leadingCombinators, \array_merge(ListUtil::exceptLast($this->components), [ListUtil::last($this->components)->withAdditionalCombinators($combinators)]), $this->lineBreak || $forceLineBreak);
    }
    /**
    ...
    public function concatenate(ComplexSelector $child, bool $forceLineBreak = \false) : ComplexSelector
    {
        if (\count($child->leadingCombinators) === 0) {
            return new ComplexSelector($this->leadingCombinators, \array_merge($this->components, $child->components), $this->lineBreak || $child->lineBreak || $forceLineBreak);
        }
        if (\count($this->components) === 0) {
            return new ComplexSelector(\array_merge($this->leadingCombinators, $child->leadingCombinators), $child->components, $this->lineBreak || $child->lineBreak || $forceLineBreak);
        }
        return new ComplexSelector($this->leadingCombinators, \array_merge(ListUtil::exceptLast($this->components), [ListUtil::last($this->components)->withAdditionalCombinators($child->leadingCombinators)], $child->components), $this->lineBreak || $child->lineBreak || $forceLineBreak);
    }
}
