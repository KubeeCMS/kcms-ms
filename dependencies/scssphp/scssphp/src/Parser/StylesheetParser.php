...
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Ast\Sass\Expression\SupportsExpression;
...
abstract class StylesheetParser extends Parser
{
    ...
    private function mediaInParens(InterpolationBuffer $buffer) : void
    {
        ...
        if ($needsParenDeprecation || $needsNotDeprecation) {
            $this->logger->warn($expression->getSpan()->message(\sprintf("Starting a @media query with \"%s\" is deprecated because it conflicts with official CSS syntax.\n\nTo preserve existing behavior: #{%s}\nTo migrate to new behavior: #{\"%s\"}\n\nFor details, see https://sass-lang.com/d/media-logic", $needsParenDeprecation ? '(' : 'not', $expression, $expression)), \true);
        }
